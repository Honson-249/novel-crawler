...
class BookDatabaseTester:
    ...
    async def test_and_store_book(self):
        ...
        author_name = self.font_mapper.decode_text(author_raw)
        ...
        if footer_elem:
            ...
            if heat_match:
                metric_value_raw = heat_match.group(1)
        ...
        if book_id:
            ...
            # 保存测试数据到 JSON
            with open("data/test_book_db.json", "w", encoding="utf-8") as f:
                json.dump(record, f, ensure_ascii=False, indent=2)
            print(f"数据已保存：data/test_book_db.json")
